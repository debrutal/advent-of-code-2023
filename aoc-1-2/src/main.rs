...
fn get_first_and_last_number(line: String) -> u32 {
    ...
    let result: String = match find_first(&line) {
        None => "0".to_string(),
        Some(first) => {
            let last = find_last(&line).expect("This should not happen since find_first already found something!");
            let number_as_str: String = if first.1 == last.1 {
                println!("adding {}", first.1);
                format!("{}", first.1).to_string()
            } else {
                println!("adding {}{}", first.1, last.1);
                format!("{}{}", first.1, last.1).to_string()
            };
